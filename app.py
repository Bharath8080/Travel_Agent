...
source = st.text_input("🛫 Departure City:", "Mumbai")  # Changed from IATA code to city name
destination = st.text_input("🛬 Destination:", "Delhi")  # Changed from IATA code to city name
...
# Validate API keys
if not SERPAPI_KEY:
    st.error("⚠️ SERPAPI_KEY is not set. Please set it in your .env file or environment variables.")
    st.stop()

if not GOOGLE_API_KEY:
    st.error("⚠️ GOOGLE_API_KEY is not set. Please set it in your .env file or environment variables.")
    st.stop()
